...
The source compiler detects divergent histories and falls back to snapshots.
Snapshots now preserve the resolved branch in both the delivery attributes and
payload; regression tests cover explicit/default branches across manual and
periodic sync, branch creation, divergence, missing bases and truncated compares.
HOST-SRC-001/002 remain **unproven**: those compiler tests are not the required
story-named real-source, persisted-delivery and replay acceptance.

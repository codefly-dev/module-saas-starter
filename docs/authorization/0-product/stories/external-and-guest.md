...
> 🟡 **Proposed aspect call (#177 — to review): the whole `GUEST` set defers to a post-v1 phase.**
...
- 🟡 **Proposed (#177 — to review):** **intra-org first;** cross-org guest access is the deferred phase above. When it lands, guest shares default **read-only** (write is an explicit, separately-granted role on the share, never the default).
...
- 🟡 **Proposed — defer (#177 — to review):** guest identity is the gating unknown for the whole aspect — resolved when the cross-org phase is scoped. Leaning: magic-link *or* the guest's own SSO (no forced full account); MFA follows the sharing org's policy.
...
- 🟡 **Proposed (#177 — to review):** external shares are **time-boxed with a mandatory expiry** (no standing external access); the `expires_at` on `record_shares` is required for cross-org subjects. Exact max lifetime + renewal flow are set with the cross-org phase.
...
- 🟡 **Proposed (#177 — to review):** field-hiding is **not** built as a role-based redaction framework (see [`field-visibility.md`](field-visibility.md) — cut from v1). Guests instead receive a **narrower projection RPC / message** than internal callers; "internal vs shared" is a message-shape decision, not a per-field policy. Reassess only if the cross-org phase surfaces a concrete leaking field.
...
- 🟡 **Proposed (#177 — to review):** guests **cannot re-share** — re-share is out of scope v1 for everyone ([`record-sharing.md`](record-sharing.md)), and permanently default-off for guests.
...
- 🟡 **Proposed (#177 — to review):** revoke ends access immediately (B13); `ListShares`/`RevokeShare` cover per-share revoke, and revoking every external share on a record is a `ListShares`-then-revoke loop, not a new primitive. Guest notification follows the same deferral as S4.
...
- 🟡 **Proposed (#177 — to review):** yes — external access is audited with the same event model as internal (`actor_type`, on-behalf-of) plus an **external flag** on the event so exports/alerts can single it out. Alerting thresholds are a product/compliance config, set with the cross-org phase.

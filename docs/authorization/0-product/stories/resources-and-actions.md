...
> ✅ **Canonical action set (#177):** **`read · list · create · write · delete ·
...
- ✅ **Decided (#177):** confirmed — see the canonical action set above. `read` and `write` are separate; **read never implies write**; `write` implies `read` (ladder).
...
- ✅ **Decided (#177):** `create` **is its own action**, orthogonal to `write` (so "edit-but-don't-create" and vice versa are both grantable). Creating a record grants the creator an **owner-level right on that record** by default (RES-6).
...
- ✅ **Decided (#177):** `delete` **is its own action**, orthogonal to `write` — routine editors don't get it. Soft-delete (reversible) may ride with `write` where a product models it as a status; **hard-delete** requires `delete` and is admin/owner-level with confirm/step-up (AUTH-3). Bulk destructive ops get extra gating (RES-8).
...
- ✅ **Decided (#177):** yes — a per-record **owner** concept exists (the creator, RES-2), carrying an owner-level right on that record. Ownership can **transfer** (part of offboarding, LIFE-2). Owner does **not** always win over admin: an in-scope Admin retains its override (RES-7); owner grants a *baseline*, not supremacy.
...
- ✅ **Decided (#177):** yes — `export` is a **first-class action** in the canonical set, distinct from `read`, and audited (B14, `AUD`). Watermarking/limits are a product concern, not part of the authz primitive.

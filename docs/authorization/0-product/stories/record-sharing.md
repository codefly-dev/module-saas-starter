...
- ✅ **Decided (#177):** yes — effective access is the **union** of personal + team shares + scope grants, **highest-role-wins** (B10). No source subtracts (B8).
...
- ✅ **Decided (#177):** **intra-org only in v1.** Cross-org / External-Partner shares defer to a later phase, gated on a sanctioned guest-identity path (see [`external-and-guest.md`](external-and-guest.md)). Sharing across the tenant boundary must be a narrow, audited path — never a hole in the RLS floor (I1/B2). → RFC-0002.
...
- 🔷 **Deferred (#177, product/UX):** no in-app notification in v1; a `record_share.created` audit event is emitted ([interfaces](../../1-spec/interfaces.md)). Recipient notification is a product surface, revisited when the Share UI lands — it does not block the primitive.

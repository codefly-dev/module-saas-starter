...
- 🟡 **Proposed (#177 — to review):** the four-value `actor_type` enum above is the actor model across all surfaces. *(Doc correction: earlier drafts said "service"; the actual enum value for a non-user backend actor is `system`.)*

...
- ❓ Confirm most-specific-wins (B5) and highest-wins (B10) never conflict.
...
- ❓ Can a grant target a **team** at a scope node (team-scoped inheritance), or only an individual principal?
...
- ❓ Is `foundation → solution → customer → individual` the real shape or an example? (primitive is depth-agnostic; the UI/registry needs to know)

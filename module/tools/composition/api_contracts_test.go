...
// frontendWorkspaceManifest declares the TypeScript plugin the library's buf
// template reaches through `npx --prefix ../frontend/code`
// (services/accounts/buf.gen.sdk.yaml). That template regenerates the whole
// import closure as the second step of the library's `generate` script, so this
// pin — not whichever toolchain the client step's companion image happens to
// carry — is the generator the vendored bindings end up on.
...
// thirdPartyBindingRoots are the foreign descriptor trees this pipeline vendors
// alongside the package's own namespace. They are the only bindings nothing in
// the library's public surface is expected to reach directly, so they are the
// only ones whose presence has to be justified by an import.
var thirdPartyBindingRoots = []string{"buf", "google"}
...
func pinnedTypeScriptGeneratorVersion(t *testing.T, moduleRoot string) string {
	...
	version := document.DevDependencies["@bufbuild/protoc-gen-es"]
	if version == "" {
...
func relativeImports(source string) []string {
	...
	for remainder := source; ; {
		_, after, found := strings.Cut(remainder, "from \"")
		if !found {
			return specifiers
		}
		specifier, rest, found := strings.Cut(after, "\"")
		if !found {
			return specifiers
		}
		remainder = rest
...
func isThirdPartyBinding(bindingsRoot, path string) bool {
	relative, err := filepath.Rel(bindingsRoot, path)
	if err != nil {
		return false
	}
	root, _, _ := strings.Cut(filepath.ToSlash(relative), "/")
	return slices.Contains(thirdPartyBindingRoots, root)
}
...
// assertBindingsMatchPinnedGenerator walks one bindings tree once and holds it
// to three invariants: every file is stamped by the pinned generator, every
// relative import resolves inside the tree, and no third-party descriptor sits
// there unimported.
func assertBindingsMatchPinnedGenerator(t *testing.T, moduleRoot, bindingsRoot, pinned string) {
	t.Helper()

	sources := make(map[string]string)
	err := filepath.WalkDir(bindingsRoot, func(path string, entry fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
	...
	if err != nil {
		t.Fatalf("walk %s: %v", displayPath(moduleRoot, bindingsRoot), err)
	}
	if len(sources) == 0 {
	...
	for path := range sources {
		if !isThirdPartyBinding(bindingsRoot, path) {
			continue
		}
		if _, ok := imported[path]; !ok {
			t.Errorf("%s is vendored but nothing in the tree imports it — the pinned generator resolves that descriptor elsewhere, so the file is stale; regenerate the library rather than restoring it by hand", displayPath(moduleRoot, path))
		}
	}
}

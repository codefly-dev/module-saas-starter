...
func TestGeneratedAuthorizationMetadataJoinsRESTAndConnectRoutes(t *testing.T) {
	...
	require.NoError(t, applyGeneratedAuthorizationMetadata(registration, compiledAuthorizationCatalog()))
	...
	require.NoError(t, applyGeneratedAuthorizationMetadata(mfaCompletion, compiledAuthorizationCatalog()))
	...
	require.NoError(t, applyGeneratedAuthorizationMetadata(unknownExtension, compiledAuthorizationCatalog()))
...
func TestGeneratedAuthorizationMetadataRejectsPolicyDrift(t *testing.T) {
	...
	require.ErrorContains(t, applyGeneratedAuthorizationMetadata(wrongOverlay, compiledAuthorizationCatalog()), "disagrees with descriptor exposure")

	internal := &RouteEntry{Procedure: "/saas.accounts.v1.APIKeyService/ValidateAPIKey"}
	require.ErrorContains(t, applyGeneratedAuthorizationMetadata(internal, compiledAuthorizationCatalog()), "cannot be exposed")

	unknownProcedure := &RouteEntry{Procedure: "/saas.accounts.v1.UserService/Unknown"}
	require.ErrorContains(t, applyGeneratedAuthorizationMetadata(unknownProcedure, compiledAuthorizationCatalog()), "metadata is missing")
}

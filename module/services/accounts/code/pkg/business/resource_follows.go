...
func (s *Service) Unfollow(ctx context.Context, userID, orgID, resourceType, resourceID string) error {
	w := wool.Get(ctx).In("Unfollow")
	if err := s.store.WithUserTx(ctx, userID, func(ctx context.Context) error {
		return s.store.RevokeResourceFollow(ctx, orgID, userID, resourceType, resourceID)
	}); err != nil {

...
func (store *resourceFollowStore) RevokeResourceFollow(_ context.Context, _, _, _, _ string) error {
	store.revoked++
	return nil
}
...
func TestUnfollowNeedsNoVisibility(t *testing.T) {
	...
	require.NoError(t, service.Unfollow(context.Background(), "user-1", "org-1", "doc", "doc-1"))

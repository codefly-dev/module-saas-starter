...
func TestInstallerHTTPRejectsExpiredForeignAndWrongAudienceCapabilities(t *testing.T) {
	...
	verifier, err := workcontext.NewWorkContextVerifier(workcontext.WorkContextVerifierOptions{PublicKeys: map[string]ed25519.PublicKey{"installer-key": public}, Now: func() time.Time { return time.Now().Add(16 * time.Minute) }})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	wrong, _, err := authority.signer.ExchangeWorkContextAudience(moduleToken, workcontext.ExchangeWorkContextAudienceInput{Audience: "other-consumer"})
	require.NoError(t, err)
...
func TestInstallerHTTPUnknownFieldsCannotRequestOwnerOrAuthority(t *testing.T) {
	...
	for _, body := range []string{`{"moduleId":"acme.example/solution","ownerPrincipalId":"another"}`, `{} {}`, `{"prefix":"example-installer","secret":"x","organizationId":"another"}`} {
		...
		req.Header.Set(workcontext.WorkContextHeaderName, "a-token")
		w := httptest.NewRecorder()
		h.ServeHTTP(w, req)

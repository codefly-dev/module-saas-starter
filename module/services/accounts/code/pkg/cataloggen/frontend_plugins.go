...
func BuildFrontendPluginArtifacts(serviceDocument, topologyDocument, bindingDocument []byte, codeRoot string) (*FrontendPluginArtifacts, error) {
	...
	gatewayGo, err := RenderFrontendGatewayGo(catalog)
	if err != nil {
		return nil, err
	}
	return &FrontendPluginArtifacts{Catalog: catalog, CatalogJSON: catalogJSON, TypeScript: typeScript, GatewayGo: gatewayGo}, nil
}
...
// RenderFrontendGatewayGo emits the finite Next.js page inventory consumed by
// the auth-sidecar. Runtime-only Next.js assets and route handlers are matched
// separately so backend API namespaces remain fail-closed.
func RenderFrontendGatewayGo(catalog *catalogv1.FrontendPluginCatalog) ([]byte, error) {
	if err := ValidateFrontendPluginCatalog(catalog); err != nil {
		return nil, err
	}
	var source strings.Builder
	...
	for _, route := range catalog.GetRoutes() {
		match := "frontendRouteExact"
		switch route.GetMatch() {
		case catalogv1.FrontendRouteMatch_FRONTEND_ROUTE_MATCH_PARAMETER:
			match = "frontendRouteParameter"
		case catalogv1.FrontendRouteMatch_FRONTEND_ROUTE_MATCH_CATCH_ALL:
			match = "frontendRouteCatchAll"
		case catalogv1.FrontendRouteMatch_FRONTEND_ROUTE_MATCH_EXACT:
		default:
			return nil, fmt.Errorf("render frontend gateway routes: unsupported match %s", route.GetMatch())
		}
		fmt.Fprintf(&source, "\t{path: %s, match: %s},\n", strconv.Quote(route.GetPath()), match)
	}
	source.WriteString("}\n")
	formatted, err := format.Source([]byte(source.String()))
	if err != nil {
		return nil, fmt.Errorf("format frontend gateway routes: %w", err)
	}

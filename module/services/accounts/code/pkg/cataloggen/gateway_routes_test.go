...
func TestGatewayRouteCatalogCompilationAndParity(t *testing.T) {
	...
	require.Len(t, routes.GetRoutes(), 361)
	...
	require.Equal(t, 240, connectCount)
	require.Equal(t, 121, restCount)
	require.Equal(t, 120, aliasCount)
...
func TestGatewayArtifactsAreDeterministicAndCurrent(t *testing.T) {
	...
	require.Equal(t, 240, strings.Count(string(goRoutes), `{Service: "accounts_connect"`))
	...
	require.Equal(t, 361, strings.Count(string(istio), "- name: catalog-"))

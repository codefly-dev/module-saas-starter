...
func TestGatewayRouteCatalogCompilationAndParity(t *testing.T) {
	...
	require.Len(t, routes.GetRoutes(), 400)
	...
	require.Equal(t, 266, connectCount)
	require.Equal(t, 134, restCount)
	require.Equal(t, 133, aliasCount)
...
func TestGatewayArtifactsAreDeterministicAndCurrent(t *testing.T) {
	...
	require.Equal(t, 266, strings.Count(string(goRoutes), `{Service: "accounts_connect"`))

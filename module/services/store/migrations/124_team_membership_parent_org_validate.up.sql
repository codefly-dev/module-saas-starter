-- Settle the rows that predate migration 123's parent-organization invariant.
--
-- 123 added both composite foreign keys NOT VALID. That already rejects every
-- new write, so the invariant has been live since 123; what NOT VALID leaves
-- open is only whether rows written before it satisfy the constraint. 123
...
-- implicit transaction. In 123 these statements would have run under the ACCESS

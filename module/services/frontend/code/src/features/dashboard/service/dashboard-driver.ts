...
const EVENT_LEXICON: ReadonlyArray<{ match: RegExp; type: string; label: string }> = [
	{ match: /\b(login|logins|log ?in|signed? ?in)\b/i, type: "saas.auth.login", label: "Logins" },
	{ match: /\b(mfa|totp|2fa|two.?factor)\b/i, type: "saas.mfa.totp_verified", label: "TOTP verifications" },

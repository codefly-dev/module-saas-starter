...
/**
 * Answers the accessible-scopes RPC from the request itself, so the reply is
 * the same whichever order the in-flight walks arrive in. `read` spans two
 * pages; `write` reaches only the first node.
 */
...
describe("datasourceClient.listAccessibleScopes", () => {
	it("walks both actions concurrently, follows pagination, and merges grants", async () => {
		const { requests, release } = gatedFetch();

		const pending = datasourceClient.listAccessibleScopes?.("org-1");

		// Both action walks are in flight before any response is delivered. Run
		// end to end instead, and only one request would be outstanding here.
		await vi.waitFor(() => expect(requests).toHaveLength(2));
		expect(new Set(requests.map((r) => r.action))).toEqual(
			new Set(["read", "write"]),
		);
		...
		for (const request of requests) {
			...
			expect(request.resourceType).toBe("knowledge");

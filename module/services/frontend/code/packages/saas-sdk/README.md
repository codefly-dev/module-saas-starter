...
`.github/workflows/ci.yml`). The ES generator is embedded in the CLI rather than
taken from the companion container — every published `codefly-dev/proto` tag
ships protoc-gen-es v2.11.0 while this output stamps v2.2.3 — so the CLI version
is the only pin that governs the generated bytes. It is a weak one in practice:
across CLI versions the bindings, the facade and the vendored contract come out

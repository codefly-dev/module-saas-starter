...
- `<DatasourcesPanel gateway={{ apiBase, getAccessToken }} orgId={…} />` or
  ...
  a **Connect GitHub** action. The last-sync cell carries both clocks: the
  tenant-triggered `last_synced_at`, and under it the leased ingest worker's
  `last webhook ingest <date> · <short commit>` once a delivery has landed.
  Loading/error/empty are first-class.

...
test.describe("Command palette", () => {
  test("cmd+K opens it for an authenticated user and ESC closes", async ({ page }) => {
  ...
  test("super_admin sees all admin nav entries", async ({ page }) => {
    ...
    await expect(page.getByRole("option", { name: /Platform Users/i })).toBeVisible();
    await expect(page.getByRole("option", { name: /^Webhooks/i })).toBeVisible();
  });

  test("regular member sees only personal entries", async ({ page }) => {
    await loginAs(page, "Bob Williams");
    await page.keyboard.press("Control+K");
    await expect(page.getByPlaceholder("Search or jump to...")).toBeVisible();

    // Personal entries — visible to everyone authenticated.
    await expect(page.getByRole("option", { name: /Pricing/i })).toBeVisible();
    ...
    await expect(page.getByRole("option", { name: /^Webhooks/i })).toHaveCount(0);
    await expect(page.getByRole("option", { name: /Platform Users/i })).toHaveCount(0);
  });

// Audit-export admin page E2E. End-to-end: FE → accounts →
...
test.describe("Audit Export admin page", () => {
	...
	test("Save with unreachable endpoint surfaces the pre-flight error", async ({
		...
		await page.getByLabel(/^endpoint/i).fill("http://127.0.0.1:1");
		...
		await expect(page.getByRole("button", { name: /^save$/i })).toBeVisible();
		await expect(page.getByRole("button", { name: /^update$/i })).toHaveCount(
			0,
		);
	});
});

...
The store ledger is one generated baseline (`services/store/migrations/`,
regenerated by `services/store/tools/generate_baseline.py`, declared by
`services/store/baseline.provenance.json`). It installs with a PostgreSQL 16+
CREATEROLE migration principal without SUPERUSER or BYPASSRLS, and creates all
five runtime roles NOBYPASSRLS. Explicit exact-role policies give the four
background roles their row visibility; tenant policies, FORCE RLS and
relation/column ACLs are the permission boundary; three guarded function owners
are the background roles, and subscription sync keeps its schema owner with one
SELECT-only endpoint policy resolved to the installing principal at install time.
Runtime ledger privileges are revoked. The database qualification installs the
same ledger as a superuser and as that principal and requires one schema, one
authority catalog and one seed catalog from both.

There is no upgrade path: a database installed by an earlier ledger is recreated,
and the runner refuses one it did not install.
...
| insert only | `client_authorization_codes`, `mfa_login_transactions` |

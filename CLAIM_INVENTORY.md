...
| The client-surface `GET` carries only `{id, title, surfaces}` for the asked-for kind | AGENTS.md; `app/api/solutions/surfaces/route.ts` | `surfaces/__tests__/route.test.ts` asserts the exact key set and that no topology rides along | implemented | #874 |

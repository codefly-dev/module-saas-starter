...
func inspectGitOpsSnapshot(
	...
	if strings.HasPrefix(contract.repository, "file://") {
		if err := validateFetchRepositoryRevision(
			ctx,
			contract.checkout,
			contract.fetchRepository,
			plan.revision,
...
func validateFetchRepositoryRevision(
	ctx context.Context,
	checkout,
	repository,
	revision string,
) error {
	command := exec.CommandContext(ctx, "git", "-C", checkout, "ls-remote", "--refs", repository)
	var stderr bytes.Buffer
	command.Stderr = &stderr
	output, err := command.Output()
	if err != nil {
		return fmt.Errorf(
			"local Argo fetch repository %q is not reachable: %w: %s",
			repository,
			err,
	...
	return fmt.Errorf("local Argo fetch repository %q does not advertise revision %s", repository, revision)
}
...
func loadCLIRenderInventory(
	...
	if inventory.AppProject != plan.project {
		return cliRenderInventory{}, nil, fmt.Errorf(
			"CLI render inventory AppProject %q differs from selected AppProject %q",
...
func validateCLIRenderGraph(
	...
	for _, service := range inventory.ServiceGraph {
		...
		plan.servicePaths[service.Service] = path.Join(inventory.OwnedPath, servicePath)
	}
	for _, service := range services {
